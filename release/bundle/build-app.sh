...
if [ -f "$ROOT/App/AppIcon.icns" ]; then
  ...
  sh "$ROOT/release/bundle/import-brand-icon.sh" glina-desktop "$RESOURCES/AppIcon.icns"
fi
...
# `build-app.sh` reached Glina only after the operator happened to quit it.
# `--if-running` leaves a stopped app stopped, and the helper launches with
# `open -g`, so nothing steals the foreground.
RESTART_APP=${WISENT_RESTART_APP:-"$ROOT/release/bundle/wisent-restart-app"}
